...
export default function SizeCreateEdit({ size }) {
    const { auth } = usePage().props
    const isEdit = Boolean(size && size.size_id)

    const { data, setData, post, patch, processing, errors, recentlySuccessful } = useForm({
        size_title: size?.size_title || '',
        user_id: size?.user_id || auth.user.id,
    })

    // Autofocus on the title field
    useEffect(() => {
        document.getElementById('size_title')?.focus()
    }, [])
    ...
    function submit(e) {
        ...
        if (isEdit) {
            patch(route('sizes.update', size.size_id), { preserveState: true })
        } else {
            post(route('sizes.store'), { preserveState: true })
        }
    }

    const title = isEdit ? 'Edit Size' : 'Create Size'
    const crumbs = [
        { title: 'Dashboard', url: route('dashboard') },
        { title: 'Size', url: route('sizes.index') },
        { title, url: '' },
    ]
        ...
        <AdminLayout breadcrumb={<Breadcrumb header={title} links={crumbs} />}>
            ...
            <div className="w-full h-screen bg-white dark:bg-gray-900 shadow-2xl rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 transition-all duration-200">
                ...
                <div className="px-8 py-6 border-b border-gray-200 dark:border-gray-800 flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {isEdit ? 'Update existing size details below' : 'Add a new size to the system'}
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="px-8 py-8 space-y-8">
                    ...
                    {/* Size Title */}
                    <div>
                        <label htmlFor="size_title" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Size Title <span className="text-red-500">*</span>
                        </label>
                        <div className="relative">
                            <input
                                ...
                                className={`block w-full rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 
                                    ...
                                    ${errors.size_title ? 'border-red-500 dark:border-red-400 focus:ring-red-200 dark:focus:ring-red-900/30' : ''}
                        ...
                        <InputError message={errors.size_title} className="mt-2 text-sm text-red-500 dark:text-red-400" />
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 rounded-xl flex justify-between items-center shadow-inner">
                        ...
                        >
                            <div className="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 shadow-md">
                                ...
                                {isEdit ? 'Size updated successfully!' : 'Size created successfully!'}
                            </div>
                        </Transition>
                            ...
                            >
                                {processing ? (
                                    ...
                                    <>
                                        ...
                                        <span>{isEdit ? 'Update Size' : 'Create Size'}</span>
                                    </>
                                )}
                            </button>
                        </div>
